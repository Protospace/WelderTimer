...
const float PRICE_PER_MINUTE = 0.6; //Dollars per minute
...
// Pins
const int LCD_PINS_RS = 12; //previous value 12
const int LCD_PINS_E = 11; // previous value 11
const int LCD_PINS_D4 = 4;
const int LCD_PINS_D5 = 3; // previous value 4
const int LCD_PINS_D6 = 2;
const int LCD_PINS_D7 = 1;
...
unsigned long totalWeldTime = 0; //time elapsed in milliseconds
unsigned long previousSessionTime;
...
void displayPreviousSession(){
  EEPROM.get(4,previousSessionTime);
  lcd.clear();
  lcd.setCursor(0,1);
  lcd.print("Previous: ");
  printTime(previousSessionTime, false, true, true);
  
  lcd.setCursor(0,2);
  lcd.print("Price: $");
  lcd.print(calculatePrice(previousSessionTime), 2);
...
void setup() {
  delay(200); //delay to let everything settle down after powering up
